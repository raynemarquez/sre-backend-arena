...
class HPApiClient:
    ...
    def __init__(self) -> None:
        ...
        self._client = httpx.AsyncClient(
            ...
            limits=httpx.Limits(max_connections=100, max_keepalive_connections=50), # Aumentado para suportar carga
        )
    ...
    @retry(
        ...
        retry=retry_if_exception_type(httpx.RequestError), # Retry apenas em erros de rede/timeout — não em 4xx/5xx
        reraise=True,
    )
    ...
    async def get_character_data(self, name: str) -> tuple[dict[str, Any], bool]:
            ...
            async with self._l2_refresh_lock:
                ...
                if not all_valid:
                    try:
                        ...
                        if is_stale and val:
                            logger.warning("fallback_stale_l1", wizard=name, error=str(exc))
                            return val, True
        ...
        found = next((c for c in all_chars if c.get("name", "").lower().strip() == cache_key), None)
...
# ------------------------------------------------------------------
    ...
    def _calculate_power_score(self, character: dict[str, Any]) -> int:
        ...
        """
        score = 50
        if character.get("wizard"): score += 20
        if character.get("house"): score += 15
    ...
    def _determine_loyalty(self, character: dict[str, Any]) -> str:
        ...
        if species == "human":
            ...
            if house == "hufflepuff": return "very_high"
            if house == "gryffindor": return "high"
            if house == "slytherin": return "self_serving"
            if house == "ravenclaw": return "intellectual"
