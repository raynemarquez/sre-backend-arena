...
class TestDetermineLoyalty:
    def test_gryffindor_human(self, hp_api_client):
        assert (
            hp_api_client._determine_loyalty(
                {"species": "human", "house": "Gryffindor"}
            )
            == "high"
        )
    ...
    def test_hufflepuff_human(self, hp_api_client):
        assert (
            hp_api_client._determine_loyalty(
                {"species": "human", "house": "Hufflepuff"}
            )
            == "very_high"
        )
    ...
    def test_slytherin_human(self, hp_api_client):
        assert (
            hp_api_client._determine_loyalty({"species": "human", "house": "Slytherin"})
            == "self_serving"
        )

    def test_human_no_house(self, hp_api_client):
        assert (
            hp_api_client._determine_loyalty({"species": "human", "house": ""})
            == "variable"
        )

    def test_house_elf(self, hp_api_client):
        assert (
            hp_api_client._determine_loyalty({"species": "house-elf", "house": ""})
            == "unconditional"
        )

    def test_goblin(self, hp_api_client):
        assert (
            hp_api_client._determine_loyalty({"species": "goblin", "house": ""})
            == "transactional"
        )

    def test_hippogriff(self, hp_api_client):
        assert (
            hp_api_client._determine_loyalty({"species": "hippogriff", "house": ""})
            == "respect-based"
        )

    def test_werewolf(self, hp_api_client):
        assert (
            hp_api_client._determine_loyalty({"species": "werewolf", "house": ""})
            == "volatile"
        )

    def test_centaur(self, hp_api_client):
        assert (
            hp_api_client._determine_loyalty({"species": "centaur", "house": ""})
            == "species-loyal"
        )

    def test_ghost(self, hp_api_client):
        assert (
            hp_api_client._determine_loyalty({"species": "ghost", "house": ""})
            == "neutral"
        )

    def test_half_giant(self, hp_api_client):
        assert (
            hp_api_client._determine_loyalty({"species": "half-giant", "house": ""})
            == "reliable"
        )

    def test_unknown_species(self, hp_api_client):
        assert (
            hp_api_client._determine_loyalty({"species": "dragon", "house": ""})
            == "instinctive"
        )
...
async def test_rate_limiter_waits_when_empty():
    ...
    with patch(
        "src.services.hp_api.asyncio.sleep", new_callable=AsyncMock
    ) as mock_sleep:
...
async def test_get_character_data_cache_miss_api_success(hp_api_client):
    ...
    with (
        ...
        patch.object(
            hp_api_client, "_fetch_with_circuit_breaker", new_callable=AsyncMock
        ) as mock_fetch,
    ):
...
async def test_get_character_data_invalid_api_response(hp_api_client):
    ...
    with patch("src.services.hp_api._cache") as mock_cache:
        ...
        with patch.object(
            ...
            with pytest.raises(
                Exception
            ):  # Esperamos que quebre pois o dado é inválido
